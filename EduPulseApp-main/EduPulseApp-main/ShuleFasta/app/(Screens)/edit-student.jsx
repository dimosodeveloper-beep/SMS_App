...
export default function EditStudent() {
  ...
  return (
    <LinearGradient colors={["#020617", "#0f172a", "#1e293b"]} style={styles.container}>
      ...
      <Header 
        title={language === "sw" ? "Hariri Mwanafunzi" : "Edit Student"} 
        subtitle={language === "sw" ? "Sasisha Taarifa za Mwanafunzi" : "Update Student"} 
      />
      ...
      <ScrollView contentContainerStyle={{ padding: 10, paddingBottom: 300 }}>
        ...
        <BlurView intensity={40} tint="dark" style={styles.blur}>
          ...
          <Text style={styles.title}>{language === "sw" ? "Hariri Mwanafunzi" : "Edit Student"}</Text>

          <View style={styles.form}>

            <Text style={styles.label}>{language === "sw" ? "Jina la Kwanza" : "First Name"}</Text>
            <TextInput style={styles.input} value={firstName} onChangeText={setFirstName} />

            <Text style={styles.label}>{language === "sw" ? "Jina la Mwisho" : "Last Name"}</Text>
            <TextInput style={styles.input} value={lastName} onChangeText={setLastName} />

            <Text style={styles.label}>{language === "sw" ? "Chagua Darasa" : "Select Classroom"}</Text>
            ...
            <Text style={styles.label}>{language === "sw" ? "Chagua Mkondo" : "Select Stream"}</Text>
            <View style={{ borderWidth: 1, borderColor: "#334155", borderRadius: 10, marginBottom: 20 }}>
              <Picker
                ...
                style={{ color: "#fff" }}
              >
                <Picker.Item label={language === "sw" ? "Chagua Mkondo" : "Select Stream"} value={null} />
                {streams.map((item) => (
                  <Picker.Item key={item.id} label={item.name} value={item.id} />
                ))}
              </Picker>
            ...
            <Text style={styles.label}>{language === "sw" ? "Tafuta Mzazi" : "Search Parent"}</Text>
            <TextInput
              style={styles.input}
              value={parentSearch}
              onChangeText={handleParentSearch}
              placeholder={language === "sw" ? "Andika jina la mzazi..." : "Type parent username..."}
            ...
            <Text style={styles.label}>{language === "sw" ? "Namba ya Usajili" : "Admission Number"}</Text>
            <TextInput style={styles.input} value={admission} onChangeText={setAdmission} />

            <Text style={styles.label}>{language === "sw" ? "Jinsia" : "Gender"}</Text>
            ...
            <Animated.View style={{ transform: [{ scale: scaleAnim }], marginTop: 20 }}>
              <TouchableOpacity onPressIn={pressIn} onPressOut={pressOut} onPress={updateStudent}>
                <LinearGradient colors={["#2563eb", "#38bdf8"]} style={styles.button}>
                  <Text style={styles.buttonText}>{language === "sw" ? "Sasisha Mwanafunzi" : "Update Student"}</Text>
                </LinearGradient>
              </TouchableOpacity>
            </Animated.View>
