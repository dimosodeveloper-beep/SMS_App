...
export default function TakeAttendance() {
  ...
  const submitAttendance = async () => {
    ...
    try {
      ...
      const res = await axios.post(EndPoint + "/take-attendance/", payload, {
        headers: {
  ...
  return (
    ...
    >
      ...
      <Header
        ...
        subtitle={language === "sw" ? "Mwanafunzi Mmoja" : "Single Student"}
      />

      <ScrollView contentContainerStyle={{ padding: 10, paddingBottom: 400 }}>
        <BlurView intensity={40} tint="dark" style={styles.blur}>
          <Text style={styles.title}>
            {language === "sw" ? "Chukua Mahudhurio" : "Take Attendance"}
          </Text>

          {/* STUDENT */}
          <Text style={styles.label}>
            {language === "sw" ? "Mwanafunzi" : "Student"}
          </Text>
          <TextInput
            style={styles.input}
          ...
          {showStudent &&
            filteredStudents.slice(0, 5).map((item) => (
              <TouchableOpacity
                key={item.id}
                style={{
          ...
          {/* DATE */}
          <Text style={styles.label}>{language === "sw" ? "Tarehe" : "Date"}</Text>
          ...
          <View style={{ flexDirection: "row", gap: 10 }}>
            <TouchableOpacity onPress={() => setStatus("present")}>
              <Text style={{ color: status === "present" ? "#00ffcc" : "#fff" }}>
                {language === "sw" ? "Ipo" : "Present"}
              </Text>
            </TouchableOpacity>

            <TouchableOpacity onPress={() => setStatus("absent")}>
              <Text style={{ color: status === "absent" ? "#ff4d4d" : "#fff" }}>
                {language === "sw" ? "Haikuwepo" : "Absent"}
              </Text>
            </TouchableOpacity>
          </View>
            ...
            >
              ...
              >
                <Text style={styles.buttonText}>
                  {language === "sw" ? "Hifadhi Mahudhurio" : "Save Attendance"}
                </Text>
              </LinearGradient>
            </TouchableOpacity>
          </Animated.View>
