...
export default function BulkAttendance() {
  ...
  const toggleStatus = (index) => {
    ...
    data[index].status =
      data[index].status === "present" ? "absent" : "present";
    setAttendance(data);
  };
  ...
  const submitBulk = async () => {
    if (!classroom || !stream) {
      Toast.show({
        type: "error",
        text1: language === "sw" ? "Chagua darasa na mkondo" : "Select class & stream",
      });
      return;
    }
    ...
    try {
      ...
      await axios.post(EndPoint + "/bulk-attendance/", payload, {
        headers: { Authorization: `Token ${token}` },
      });

      Toast.show({
        type: "success",
        text1: language === "sw" ? "Mahudhurio yamehifadhiwa" : "Attendance saved",
      });
  ...
  return (
    ...
    >
      ...
      <Header
        title={language === "sw" ? "Mahudhurio ya Pamoja" : "Bulk Attendance"}
        subtitle={language === "sw" ? "Darasa Zima" : "Whole Class"}
      />

      <ScrollView contentContainerStyle={{ padding: 10, paddingBottom: 500 }}>
        <BlurView intensity={40} tint="dark" style={styles.blur}>
          <Text style={styles.title}>
            {language === "sw" ? "Mahudhurio ya Darasa" : "Class Attendance"}
          </Text>
          ...
          <TouchableOpacity onPress={submitBulk}>
            <Text style={{ color: "#38bdf8", textAlign: "center", marginTop: 20 }}>
              {language === "sw" ? "Wasilisha Mahudhurio" : "Submit Attendance"}
            </Text>
          </TouchableOpacity>
        </BlurView>
      </ScrollView>
