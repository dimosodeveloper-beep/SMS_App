...
export default function AllGradings() {
  ...
  const fetchGradings = async () => {
    ...
    try {
      ...
      Toast.show({
        type: "error",
        text1: language === "sw" ? "Hitilafu kupata daraja" : "Error fetching grading"
      });
    }
    setLoading(false);
  };
  ...
  const deleteGrading = async () => {
    try {
      ...
      Toast.show({ 
        type: "success", 
        text1: language === "sw" ? "Imeondolewa" : "Deleted" 
      });
  ...
  return (
    ...
    <LinearGradient colors={["#020617", "#0f172a", "#1e293b"]} style={styles.container}>
      ...
      <Header 
        title={language === "sw" ? "Mfumo wa Daraja" : "Grading System"} 
        subtitle={language === "sw" ? "Jedwali la Daraja Zote" : "All Grades Table"} 
      />
      ...
      <ScrollView contentContainerStyle={{ padding: 10, paddingBottom: 300 }}>
        ...
        <BlurView intensity={40} tint="dark" style={styles.blur}>
          ...
          <Text style={styles.title}>{language === "sw" ? "Jedwali la Daraja" : "Grading Table"}</Text>
        ...
        <View style={{
          ...
          <View style={{
            ...
            <Text style={{ color: "#fff", marginBottom: 20 }}>
              {language === "sw" ? "Je, una uhakika unataka kufuta daraja hili?" : "Are you sure you want to delete this grading?"}
            </Text>

            <View style={{ flexDirection: "row", justifyContent: "space-between" }}>

              <TouchableOpacity onPress={() => setDeleteModal(false)}>
                <Text style={{ color: "#94a3b8" }}>{language === "sw" ? "Ghairi" : "Cancel"}</Text>
              </TouchableOpacity>

              <TouchableOpacity onPress={deleteGrading}>
                <Text style={{ color: "#ef4444", fontWeight: "bold" }}>{language === "sw" ? "Futa" : "Delete"}</Text>
              </TouchableOpacity>
